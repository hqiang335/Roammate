...
LIGHTWEIGHT_TYPES = ["transport", "hotel", "restaurant", "meal", "airport", "station", "交通", "酒店", "餐", "机场", "车站"]
...
def validate_day(day: dict[str, Any], flags: dict[str, bool]) -> list[str]:
    ...
    for index, poi in enumerate(pois, start=1):
        ...
        type_text = str(poi.get("type") or "").lower()
        is_lightweight = any(token in type_text for token in LIGHTWEIGHT_TYPES)
        if not is_lightweight:
            if poi.get("comfortable_duration_minutes") is None:
                issues.append(f"Day {day_no} POI '{name or index}': missing comfortable_duration_minutes.")
            if not poi.get("must_do"):
                issues.append(f"Day {day_no} POI '{name or index}': missing must_do.")
            if not poi.get("queue_or_friction"):
                issues.append(f"Day {day_no} POI '{name or index}': missing queue_or_friction.")
            if not poi.get("source"):
                issues.append(f"Day {day_no} POI '{name or index}': missing source.")
...
def validate(data: dict[str, Any]) -> list[str]:
    ...
    if not (data.get("start_date") or data.get("date_range")):
        issues.append("Trip: missing start_date or date_range.")
