...
- `itinerary-data.json` must be strict JSON. Do not leave unescaped `"` inside string values.
- Prefer short arrays for `must_do`, `queue_or_friction`, `preparation`, and `source` instead of long prose strings with nested quotes.
