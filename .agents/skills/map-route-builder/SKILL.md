...
description: Convert mainland China place lists or itinerary POIs into normalized POI JSON, route summaries, transport/hotel/ticket references, lightweight map artifacts, and Amap/FlyAI/Quark-backed travel data.
---
...
4. For full-package map artifacts, **must run** `scripts/build_real_map.py`. It uses Amap Web Service API for POIs/routes and FlyAI CLI for hotel inventory results.
5. Validate the generated map with `scripts/validate_map.mjs`.
...
10. Generate `map-data.json`, legacy `pois.json`, route summary, hotel candidates, transport/ticket notes, and real standalone dashboard `map.html`.
...
- Do not hand-write `map.html`. A valid `map.html` must be generated by `scripts/build_real_map.py` and pass `scripts/validate_map.mjs`.
- If the real map script fails because keys, network, Amap, or FlyAI are unavailable, do not create a placeholder named `map.html`; write `map-error.md` plus route text instead.
...
- If the map cannot be generated, still output normalized POIs and route text. The standalone `map.html` should remain useful on its own, but the richer final experience is the generated `guidebook.html` Travel Atlas, which merges map data with itinerary and reputation dossiers.
