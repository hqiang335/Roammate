...
`guidebook-data.json` is generated by `scripts/build-guidebook-data.mjs` and rendered by `scripts/build-guidebook.mjs`. Keep it structured, compact, and focused on guidebook decisions; the renderer also reads sibling `map-data.json` and Markdown source files when present.

Build it primarily from `itinerary-data.json`, `map-data.json`, and the three Markdown reports. Use `research-ledger.json` for audit/source IDs only when needed.
...
- Main modules should carry the decision facts: overall plan, day-by-day route, POI reputation, hotels, budget, food, checklist, and inline freshness notes.
...
- inline source notes for volatile or experience-based judgments, with detailed references preserved in `research-ledger.json` and collapsed source documents.
