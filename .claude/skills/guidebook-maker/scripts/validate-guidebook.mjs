...
if (html) {
  ...
  for (const required of ["Roammate Travel Atlas", "行程总览", "总体安排", "每日安排", "地图与交通", "来源与可信度"]) {
...
if (data && html) {
  ...
  for (const [index, source] of asArray(data.sources || data.sourceNotes).entries()) {
    if (source?.url && !html.includes(String(source.url))) {
      issues.push(`sources[${index}] URL missing from guidebook.html: ${source.url}`);
    }
  }
  for (const day of getDays(data)) {
  ...
  for (const factId of asArray(data.sourceFactIds)) {
    if (factId && !html.includes(String(factId))) {
      issues.push(`sourceFactId missing from guidebook.html: ${factId}`);
    }
  }
}
