...
function collectMarkdownBullets(block, heading) {
  const inline = collectMarkdownInlineField(block, heading);
  if (inline) return inline.split(/[；;]\s*/).map(stripMarkdown).filter(Boolean);
  const lines = block.split(/\r?\n/);
  const items = [];
  let active = false;
  for (const line of lines) {
    if (line.includes(`**${heading}**`) || new RegExp(`^\\s*-?\\s*${heading}\\s*[:：]?$`).test(stripMarkdown(line))) {
      active = true;
      continue;
    }
    if (!active) continue;
    if (/^\s*(---|###\s+|\*\*[^*]+\*\*)/.test(line)) break;
    if (/^\s*-?\s*[^：:]{2,24}[:：]/.test(line) && !line.includes(heading)) break;
...
function collectMarkdownInlineField(block, heading) {
  ...
  const line = block.match(new RegExp(`^\\s*-?\\s*${heading}[:：]\\s*(.+)$`, "m"));
...
function placeNameMatches(a, b) {
  ...
  if (/^(成都|广州|上海|北京|机场|车站|酒店|市区|附近)$/.test(left) || /^(成都|广州|上海|北京|机场|车站|酒店|市区|附近)$/.test(right)) {
    return false;
  }
...
function durationText(value) {
  ...
  if (typeof value === "number" && Number.isFinite(value)) return `约${value}分钟`;
  const text = String(value).trim();
  return /^\d+(?:\.\d+)?$/.test(text) ? `约${text}分钟` : text;
}
...
function isTransferOrLogisticsActivity(activity) {
  const title = String(activity?.title || "");
  if (/→|前往|出发|抵达|到达|返回|去往/.test(title)) return true;
  if (/办理入住|退房|取行李|购买伴手礼|最后打卡/.test(title)) return true;
  return /^(早餐|午餐|晚餐)$/.test(title);
}

function scoreActivityMatch(primary, candidate) {
  if (isTransferOrLogisticsActivity(primary) && !isTransferOrLogisticsActivity(candidate)) return 0;
  let score = 0;
...
function mergeActivityLists(primaryRaw, fallbackRaw, knownNames = []) {
  ...
  const primaryFromMarkdown = primary.some((activity) => /itinerary\.md/.test(String(activity.source || "")));
  ...
  const merged = primary.map((activity) => {
    ...
    return {
      ...
      duration: durationText(firstPresent(activity.duration, candidate?.duration, durationFromTimeRange(activity.time || candidate?.time))),
  ...
  fallback.forEach((activity, index) => {
    if (primaryFromMarkdown && !/itinerary\.md/.test(String(activity.source || ""))) return;
...
function parseItineraryMarkdown(text, knownNames = []) {
  ...
  const matches = [...text.matchAll(/^###\s+Day\s+(\d+)(?:\s*[（(][^）)]*[）)])?\s*(?:[·.-]\s*)?(.+)$/gm)];
  const days = new Map();
  for (let index = 0; index < matches.length; index += 1) {
    ...
    days.set(day, {
      day,
      theme: title.replace(/[（(][^）)]*(?:\d{4}|月|周|星期)[^）)]*[）)]/g, "").trim(),
      activities,
...
function parseReputationMarkdown(text) {
  ...
  for (let index = 0; index < matches.length; index += 1) {
    ...
    const block = text.slice(match.index, next?.index ?? text.length);
    if (/餐厅推荐|不辣|川菜体验|住宿|酒店|候选|区域判断|推荐档次|交通|预算/.test(name)) continue;
    map.set(name, {
      name,
...
function normalizeActivity(activity, knownNames = []) {
  ...
  const duration = firstPresent(activity.duration, activity.estimated_duration_minutes, activity.comfortable_duration_minutes);
  return {
...
function normalizeDays(data, itineraryText) {
  const guideDays = asArray(firstPresent(data.days, data.itinerary?.days));
  const names = knownPoiNames(data);
  const hasStructuredActivities = guideDays.some((day) => asArray(firstPresent(day?.activities, day?.timeline, day?.pois)).length);
  const markdownDays = hasStructuredActivities ? new Map() : parseItineraryMarkdown(itineraryText, names);
  const count = Math.max(guideDays.length, markdownDays.size);
  return Array.from({ length: count }, (_, index) => {
    const day = guideDays[index] || {};
    const markdown = markdownDays.get(Number(firstPresent(day.day, index + 1))) || {};
    const rawActivities = asArray(firstPresent(day.activities, day.timeline, day.pois));
    const fallbackActivities = asArray(markdown.activities);
    const activities = mergeActivityLists(rawActivities, fallbackActivities, names);
    return {
      day: Number(firstPresent(day.day, markdown.day, index + 1)),
      date: firstPresent(day.date, day.dayDate),
      theme: firstPresent(day.theme, day.title, markdown.theme, `第 ${index + 1} 天`),
      area: firstPresent(day.area, day.region),
      intensity: firstPresent(day.intensity, day.pace),
      highlights: asArray(day.highlights),
      meals: firstPresent(day.meals, day.food, markdown.meals),
      rest: firstPresent(day.rest, day.restNotes, markdown.rest),
      budget: firstPresent(day.budget, day.costs, markdown.budget),
      reservations: firstPresent(day.reservations, day.booking, day.reservationReminders, markdown.reservations),
      backup: firstPresent(day.backup, day.rainPlan, day.alternative),
      reminders: asArray(firstPresent(day.reminders, day.warnings, markdown.reminders)),
      activities,
    };
  });
}

function normalizePoiExperiences(data, days, reputationText = "", mapData = {}) {
  ...
  const mapPois = asArray(mapData?.pois);
  if (!cards.length) {
    for (const day of asArray(data.days)) {
      for (const poi of asArray(day?.pois)) {
  ...
  return cards.filter((item) => item?.name && !/住宿|酒店|候选|区域判断|推荐档次/.test(String(item.name))).map((item) => {
    const rep = reputation.get(String(item.name))
      ...
      || {};
    const mapPoi = mapPois.find((poi) => {
      const candidates = [poi.name, poi.original_name, poi.originalName, poi.title].filter(Boolean);
      const itemNames = String(item.name || "").split(/[、/]/).map((part) => part.trim()).filter(Boolean);
      return candidates.some((candidate) => placeNameMatches(item.name, candidate) || itemNames.some((part) => placeNameMatches(part, candidate)));
    }) || {};
    return {
      name: item.name,
      day: lookupDayForName(item.name, dayByName) || "",
      rating: firstPresent(item.rating, item.grade),
      image: firstPresent(item.image, item.mainPic, item.photo, poiImage(mapPoi)),
...
function lookupDayForName(name, lookup) {
  const target = comparableText(name);
  if (!target) return "";
  for (const [candidate, day] of lookup.entries()) {
    if (candidate === name) return day;
    const key = comparableText(candidate);
    if (!key) continue;
    if ((target.length >= 3 && key.includes(target)) || (key.length >= 3 && target.includes(key))) return day;
  }
  return "";
}

function coordinatesFromPoi(poi) {
  if (Array.isArray(poi?.location)) return poi.location;
  const lng = Number(firstPresent(poi?.longitude, poi?.lng, poi?.coordinates?.longitude, poi?.coordinates?.lng));
  const lat = Number(firstPresent(poi?.latitude, poi?.lat, poi?.coordinates?.latitude, poi?.coordinates?.lat));
  return Number.isFinite(lng) && Number.isFinite(lat) ? [lng, lat] : null;
}

function poiImage(poi) {
  return firstPresent(poi?.image, poi?.mainPic, poi?.photo, asArray(poi?.photos)[0]?.url);
}

function mergeMapPoi(existing, incoming) {
  if (!existing) return incoming;
  return {
    ...existing,
    originalName: firstPresent(existing.originalName, incoming.originalName),
    address: firstPresent(existing.address, incoming.address),
    location: firstPresent(existing.location, incoming.location),
    day: firstPresent(existing.day, incoming.day),
    category: firstPresent(existing.category, incoming.category),
    marker: firstPresent(existing.marker, incoming.marker),
    confidence: firstPresent(existing.confidence, incoming.confidence),
    source: firstPresent(existing.source, incoming.source),
    image: firstPresent(existing.image, incoming.image),
  };
}

function addMapPoi(pois, poi) {
  if (!poi?.name) return;
  const index = pois.findIndex((item) => {
    const left = comparableText(item.originalName || item.name);
    const right = comparableText(poi.originalName || poi.name);
    return left && right && (left === right || left.includes(right) || right.includes(left));
  });
  if (index >= 0) {
    pois[index] = mergeMapPoi(pois[index], poi);
  } else {
    pois.push(poi);
  }
}
...
function normalizeTransportRows(data, itineraryText = "") {
  ...
  const flightRows = asArray(transport.flights).map((flight) => ({
    type: firstPresent(flight.type, flight.direction, "航班"),
    route: firstPresent(
      flight.route,
      [flight.direction, flight.option, flight.flightNo, flight.flight_no, flight.trainNo, flight.train_no].filter(Boolean).join(" · "),
      [firstPresent(flight.dep_airport, flight.depAirport, flight.departure, flight.from), firstPresent(flight.arr_airport, flight.arrAirport, flight.arrival, flight.to)].filter(Boolean).join(" → "),
    ),
    duration: firstPresent(flight.duration, flight.time, flight.duration_min ? `约${flight.duration_min}分钟` : ""),
    cost: firstPresent(flight.price, flight.cost, flight.price_cny ? `¥${flight.price_cny}` : ""),
    source: firstPresent(flight.source, flight.notes, flight.note),
  ...
  const trainRows = asArray(transport.trains).map((train) => ({
    type: firstPresent(train.type, train.direction, "高铁/火车"),
    route: firstPresent(
      train.route,
      [train.direction, train.option, train.trainNo, train.train_no, train.flightNo, train.flight_no].filter(Boolean).join(" · "),
      [firstPresent(train.dep_station, train.depStation, train.departure, train.from), firstPresent(train.arr_station, train.arrStation, train.arrival, train.to)].filter(Boolean).join(" → "),
    ),
    duration: firstPresent(train.duration, train.time, train.duration_min ? `约${train.duration_min}分钟` : ""),
    cost: firstPresent(train.price, train.cost, train.price_cny ? `¥${train.price_cny}` : ""),
    source: firstPresent(train.source, train.notes, train.note),
  ...
  const localRows = asArray(transport.local).map((item) => {
    ...
    return {
      ...
      route: firstPresent(item.route, item.name, item.summary, item.note, item.notes),
      duration: firstPresent(item.duration, item.time),
      cost: firstPresent(item.price, item.cost),
      source: firstPresent(item.source, item.notes, transport.source),
      url: firstPresent(item.url, item.bookingUrl, item.booking_url, item.ticketUrl, item.ticket_url, item.detailUrl, item.jumpUrl),
    };
  });
  const hasStructuredTransport = rows.length || flightRows.length || trainRows.length || localRows.length;
  const markdownRows = hasStructuredTransport ? [] : parseTransportMarkdown(itineraryText);
  const allRows = [...rows, ...flightRows, ...trainRows, ...localRows, ...markdownRows]
    .map((row) => ({
      type: firstPresent(row.type, row.category, "交通"),
      route: firstPresent(row.route, row.name, row.summary, row.note, row.notes),
...
function normalizeMapPayload(mapData, days, hotels, routeSegments = []) {
  const dayByName = dayNameLookup(days);
  const pois = [];
  asArray(mapData?.pois).forEach((poi, index) => {
    const name = firstPresent(poi.name, poi.original_name, `地点 ${index + 1}`);
    const originalName = firstPresent(poi.original_name, poi.originalName);
    const day = lookupDayForName(originalName, dayByName) || lookupDayForName(name, dayByName) || "";
    ...
    addMapPoi(pois, {
      ...
      originalName,
      address: poi.address,
      location: coordinatesFromPoi(poi),
      day,
...
function isMovementActivity(activity) {
  return String(activity.type || "") === "transport"
    || isTransferOrLogisticsActivity(activity)
...
function transportGroupKey(row) {
  ...
  const text = `${type} ${route}`;
  if (/市内|城内|当地|本地|地铁|公交|出租|打车|网约车|步行|接驳|景区直通车/.test(text)) return "local";
  if (/返程|回程|返航|返回|回到|回家/.test(text)) return "return";
  if (/去程|出发|前往|抵达目的地/.test(text)) return "outbound";
  if (/航班|飞机|高铁|火车|动车|车次|机场|车站/.test(text)) return "intercity";
  return "other";
}

const transportGroupMeta = {
  outbound: { label: "去程", hint: "抵达目的地的城际交通" },
  return: { label: "返程", hint: "离开目的地的城际交通" },
  intercity: { label: "城际备选", hint: "未标明方向的航班/车次" },
  local: { label: "市内交通", hint: "到站接驳、地铁、公交和打车建议" },
  other: { label: "其他参考", hint: "补充交通信息" },
};

function groupedTransportRows(rows) {
  ...
  return ["outbound", "return", "intercity", "local", "other"]
    .filter((key) => groups.has(key))
    .map((key) => ({ key, ...transportGroupMeta[key], rows: groups.get(key) }));
}

function renderTransportRows(rows) {
  if (!rows.length) return "";
  const groups = groupedTransportRows(rows);
  return `
      ...
      <div class="dossier-grid">
        ${experiences.map((item) => `
          <button class="dossier-card dossier-button" type="button" data-focus-poi="${escapeHtml(item.name)}" data-dossier="${escapeHtml(item.name)}">
            ${item.image ? `<img src="${escapeHtml(item.image)}" alt="${escapeHtml(item.name)}">` : `<div class="dossier-image-fallback">地点档案</div>`}
            <span>地点档案${item.day ? ` · Day ${escapeHtml(item.day)}` : ""}</span>
...
function renderSourceDocuments(documents) {
  ...
  return `
    <section class="panel docs-intro" id="full-docs">
      ...
      <h2>原文归档：需要核对时再展开</h2>
      <p class="lead">主页面优先呈现可执行信息；原始简报、行程和口碑保留为折叠归档，方便核对来源。</p>
      <div class="doc-jump">
...
function css() {
  return `
    ...
    * { box-sizing: border-box; }
    ...
    .transport-groups { display: grid; gap: 16px; }
    .transport-group { padding: 14px; border: 1px solid var(--line); border-radius: var(--radius); background: #fffdf8; }
    .transport-group-head { display: flex; align-items: baseline; justify-content: space-between; gap: 12px; margin-bottom: 10px; }
    .transport-group-head h3 { margin: 0; font-size: 18px; }
    .transport-group-head span { color: var(--muted); font-size: 13px; }
    ...
    .transport-card h4 { margin: 4px 0 6px; font-size: 15px; line-height: 1.45; }
    ...
    .dossier-button { width: 100%; min-height: 148px; display: grid; gap: 4px; align-content: start; padding: 0 14px 14px; text-align: left; cursor: pointer; color: inherit; transition: transform .18s ease, box-shadow .18s ease, border-color .18s ease; }
    ...
    .dossier-button img, .dossier-image-fallback { width: calc(100% + 28px); max-width: none; height: 142px; margin: 0 -14px 10px; object-fit: cover; background: #efe4d5; }
    .dossier-image-fallback { display: grid; place-items: center; color: var(--accent-2); font-size: 13px; font-weight: 900; }
    .dossier-button span { margin-top: 14px; color: var(--accent-2); font-size: 12px; font-weight: 800; }
    .dossier-button img + span, .dossier-image-fallback + span { margin-top: 0; }
  ...
  return `
    ...
    function setDay(day) {
      ...
      if (map) {
        ...
        const visiblePoints = [...activePois, ...data.map.hotels.filter((item) => Array.isArray(item.location))];
        ...
        if (routeLine) {
          if (activePois.length > 1) {
            routeLine.setPath(activePois.map((item) => item.location));
            routeLine.setMap(map);
          } else {
            routeLine.setMap(null);
          }
        }
        const activeMarkers = visiblePoints.map((item) => markers.get(item.name)).filter(Boolean);
...
// itinerary-data.json is intentionally ignored. The guidebook content pipeline is
// Markdown-authoritative: itinerary.md/reputation.md/destination-brief.md provide
// the traveler-facing content. itinerary-structured.json is a lossless index of
// itinerary.md, while map-data.json only enriches with map facts.
...
const days = normalizeDays(data, itineraryText);
const planRows = parseItineraryOverview(itineraryText);
const hotels = normalizeHotels(data, mapData);
const poiExperiences = normalizePoiExperiences(data, days, reputationText, mapData);
