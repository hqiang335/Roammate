...
4. Read `references/itinerary-rules.md`. In a full-package run, reuse any shared concierge references already loaded by the router; only open `tool-priority.md`, `ai-search-playbook.md`, `research-ledger-schema.md`, or `data-flow.md` if they have not been read yet or a validator failure requires the exact contract.
