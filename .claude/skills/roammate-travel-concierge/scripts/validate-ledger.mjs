...
if (!isObject(ledger)) {
  ...
  for (const [index, run] of asArray(ledger.tool_runs).entries()) {
    ...
    if (
      ...
      issues.push(`${label} Web-Rooter run must preserve usable citation URLs, source_urls, urls, or references_text in final mode; citation IDs such as W1 are not enough.`);
    }
  }
