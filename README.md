...
Roammate 是一个基于 Claude Code 的智能旅行规划系统，专注于中国大陆旅行。仓库提交的 skill 实现统一放在 `.claude/skills`；根目录的 `AGENTS.md` 让 Codex / StudyClawHub 也能读取同一套项目说明。系统通过项目级 skills 协调目的地调研、口碑研究、行程规划、地图路线和交互旅行地图册生成；配置高德地图 API、FlyAI 和 Web-Rooter 后，可以生成更完整的 POI、路线、互动地图、酒店参考，并通过 Web-Rooter 获取带引用的公开网页证据、官方公告、游客口碑和餐厅口碑。
...
4. **map-route-builder** - 地图路线
   ...
   - 可复用地图数据生成，供 `guidebook.html` 内嵌地图使用
...
│       ├── guidebook.html
│       └── sources.md
├── AGENTS.md                      # Agent / Codex / StudyClawHub 入口说明
├── CLAUDE.md                      # 项目说明文档
├── package.json                   # Node.js 依赖
└── README.md                      # 本文件
```

本仓库只提交 `.claude/skills` 作为唯一 skill 实现，避免维护两份重复代码。若本地出现 `.agents/`，它只是某些 Codex/agent 环境生成或使用的本地镜像目录，不需要上传到 GitHub。`AGENTS.md` 和 `CLAUDE.md` 保持相同项目规则是正常的：前者给 Agent / Codex / StudyClawHub 读取，后者给 Claude Code 读取。
...
如果需要运行 `guidebook.html` 的浏览器渲染 QA，可以安装 Playwright 浏览器：
...
- **map-data.json** - 景点、坐标、路线和酒店的结构化地图数据，供 `guidebook.html` 内嵌地图使用
- **pois.json** - V1 兼容别名，内容同 `map-data.json`
