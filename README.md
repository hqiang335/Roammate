...
Roammate 是一个基于 Claude Code / Codex 的智能旅行规划系统，专注于中国大陆旅行。它通过项目级 skills 协调目的地调研、口碑研究、行程规划、地图路线数据和交互旅行地图册生成；配置高德地图 API、FlyAI 和 Web-Rooter 后，可以生成更完整的 POI、路线、guidebook 内嵌地图、酒店参考，并通过 Web-Rooter 获取带引用的公开网页证据、官方公告、游客口碑和餐厅口碑。

## 在线 Demo

GitHub Pages 部署后，可以直接在浏览器或手机中打开示例旅行地图册。当前 demo 对应本仓库内的 `TRAVEL/成都-2026-06-01/guidebook.html`：

- [成都 2026-06-01 旅行地图册](https://hqiang335.github.io/Roammate/TRAVEL/%E6%88%90%E9%83%BD-2026-06-01/guidebook.html)
- [本地源文件](TRAVEL/成都-2026-06-01/guidebook.html)
...
3. **itinerary-planner** - 行程规划
   ...
   - 家庭出行、自驾、预算优化
   - 雨天备用方案
   - 从 `itinerary.md` 生成无损机器索引 `itinerary-structured.json`

4. **map-route-builder** - 地图路线
   - POI 规范化和地理编码
   - 高德地图路线规划
   - 已接受酒店候选的坐标补全和路线摘要
   - guidebook 内嵌地图所需的数据生成

5. **guidebook-maker** - 交互旅行地图册
   - dashboard 型 HTML Travel Atlas
   - 地图、每日行程、地点详情、酒店、预算和折叠来源档案
   - 稳定视觉系统，支持浏览器打印
...
- Web-Rooter `wr` CLI（可选/按需），用于少量可引用公共网页证据、官方页面读取和冲突复核

基础文本规划可以在没有高德/FlyAI/Web-Rooter 的情况下运行，但地图、路线、酒店和公共网页证据需要额外配置。
...
如果需要运行 guidebook 的浏览器渲染 QA，可以安装 Playwright 浏览器：
...
| `AMAP_WEB_JS_API_KEY` | 推荐 | 高德 Web JS API，用于生成的 `guidebook.html` 前端地图渲染 | 在高德开放平台创建 Web 端应用 Key |
...
| `wr` CLI（Web-Rooter） | 可选/按需 | 官方公告、少量公共网页证据、游客避坑、酒店/餐厅区域口碑复核 | 安装 [web-rooter](https://github.com/baojiachen0214/web-rooter) |
...
Web-Rooter 是本项目的按需公共网页证据补强层，用来替代自动使用 Claude Code 内置 Web Search。它适合做“搜索 → 少量阅读 → 引用 → 可复查”的链路，输出中的 `citations` 和 `references_text` 可写入 `research-ledger.json`。在旅行规划里，Amap/FlyAI 仍是默认结构化来源；Web-Rooter + Quark 只在官方公告、信息冲突、真实避坑、住宿区域、餐厅口碑、复杂景点玩法等 Amap/FlyAI 不够用的场景补证据。
...
本项目默认只使用公开、非登录网页。网络搜索统一使用 Quark，但只作为按需证据补强层：先用 `npm run --silent wr -- web ... --engine=quark --no-crawl --num-results=3 | npm run --silent wr:compact` 看前几个可引用结果，再按需用项目 wrapper 调用 `visit` / `html --max-chars=8000` 精读 1 个可信 URL。普通流程默认不使用 `deep` 或 `do`；只有多源冲突、官方信息难找或用户明确要求深度研究时才用。酒店、餐厅和复杂景点攻略可以用 Quark 校正 Amap/FlyAI，但不要让 WR 原始输出进入上下文或产物。不要使用小红书、`wr xhs`、账号登录、Cookie 抓取、点赞评论、发帖、预订或付款。
...
npm run --silent wr -- web "成都 大熊猫繁育研究基地 官方 预约 开放时间" --engine=quark --no-crawl --num-results=3 --command-timeout-sec=60 \
...
```bash
# 快速查一个点，先拿前 3 条引用搜索结果
npm run --silent wr -- web "{景点} 官方 预约 开放时间" --engine=quark --no-crawl --num-results=3 --command-timeout-sec=60 \
  | npm run --silent wr:compact

# 找到可信 URL 后再读页面
npm run --silent wr -- visit "{url}" | npm run --silent wr:compact
npm run --silent wr -- html "{url}" --max-chars=8000 | npm run --silent wr:compact

# 搜索并抓取 1 个页面，只有 snippets 不够时使用
npm run --silent wr -- web "{景点} 值得去吗 避雷 排队 亲子" --engine=quark --crawl-pages=1 --num-results=3 --command-timeout-sec=90 \
  | npm run --silent wr:compact

# 多源交叉验证，仅在冲突或用户明确要求时使用
npm run --silent wr -- deep "{城市} 亲子游 避雷 餐厅 住宿 区域" --engine=quark --variants=2 --crawl=1 --command-timeout-sec=120 \
  | npm run --silent wr:compact

# 酒店真实经验层：先判断住哪片区和档次组合，再回到 FlyAI 查库存/价格
npm run --silent wr -- web "{城市} 亲子 住哪里方便 酒店 推荐 档次 区域 避雷" --engine=quark --no-crawl --num-results=3 --command-timeout-sec=60 \
  | npm run --silent wr:compact
npm run --silent wr -- web "{城市} {区域} 酒店 推荐 亲子 早餐 隔音 交通 避雷" --engine=quark --crawl-pages=1 --num-results=3 --command-timeout-sec=90 \
  | npm run --silent wr:compact
```
...
2. 使用主路由 skill 开始规划：
   ```
   /roammate-travel-concierge 广州出发，2026年6月1日去成都玩4天3夜，一家三口含7岁小朋友，飞机往返，节奏轻松，生成完整旅行包和 guidebook。
   ```

   如果 slash command 暂时没有出现，也可以直接输入完整需求：
   ```
   广州出发，2026年6月1日去成都玩4天3夜，一家三口含7岁小朋友，飞机往返，节奏轻松。请按 Roammate 完整五步流程生成所有产物。
   ```

3. 主路由 skill 只是轻量串行调度器：不会启动子 agent，不会预读所有子 skill 或所有参考文件。系统会按顺序执行完整五步流程：
   - 生成目的地简报
   - 进行口碑调研
   - 规划详细行程
   - 构建地图和路线
...
4. 所有产出物保存在 `TRAVEL/{目的地}-{YYYY-MM-DD}/` 目录。同一次行程只使用一个 canonical 目录，例如 `TRAVEL/成都-2026-06-01/`；不要混用 `TRAVEL/成都-20260601/` 这类紧凑日期目录。

流程会先把可复用事实写入 `research-ledger.json`，再以 `itinerary.md` 作为行程权威生成无损索引 `itinerary-structured.json`，随后生成 `map-data.json`。`guidebook-data.json` 由 `guidebook-maker/scripts/build-guidebook-data.mjs` 从三大 Markdown 报告和无损索引中整理生成，`guidebook.html` 由 `guidebook-maker/scripts/build-guidebook.mjs` 渲染，并自动合并同目录的 `map-data.json`。如果外部工具或密钥不可用，应输出降级说明，而不是把占位页冒充正式手册。完整流程不得在第 3 步行程规划后输出最终总结；只有地图数据、手册和 `validate:trip` 全部完成或明确降级后，才算交付。
...
```
/destination-brief 成都，2026年6月1日出发，4天3夜，广州出发，一家三口含7岁小朋友
/local-reputation-research 成都，熊猫基地、都江堰、宽窄巷子、人民公园、杜甫草堂
/itinerary-planner 成都，2026年6月1日，4天3夜，一家三口含7岁小朋友，广州出发
/map-route-builder 成都，熊猫基地、都江堰、宽窄巷子、人民公园、杜甫草堂、春熙路
/guidebook-maker 成都，基于 TRAVEL/成都-2026-06-01/ 生成手册
```
...
- **Playwright** - guidebook 浏览器渲染 QA（可选）
...
- **不代替用户操作** - 不预订、付款或登录
- **结构化优先，网页小剂量补证据** - Amap/FlyAI 负责位置、路线、库存和价格，Web-Rooter + Quark 只在需要官方/口碑/避坑复核时少量使用；Claude Code 内置 Web Search 不自动兜底
...
- 本项目专注于中国大陆旅行规划
- 配置高德地图和 FlyAI 可获得更完整的路线、酒店和交通数据；Web-Rooter 仅用于按需网页证据。缺失时会降级为估算、文本建议或标注待核实，不自动使用 Claude Code 内置 Web Search 兜底
